...
# 줄바꿈 없이 한 줄로 정렬
MODEL_SHORT = ["Claude", "GPT-5.5", "Gemini", "Grok"]
MODEL_PLAIN = ["Claude Opus 4.8", "GPT-5.5", "Gemini 3.1 Pro", "Grok 4.3"]

# 출처별 데이터
# SWE-bench: Anthropic System Card, OpenAI, Google
# Terminal-Bench / OSWorld: Anthropic System Card, HundredTabs
# GPQA / HLE: Anthropic System Card, Vellum
# Math: AI Magic, Vellum
DATA = {
...
def source_footer(slide, src_text):
    """각 슬라이드 하단에 출처 표기"""
    text(slide, Inches(0.5), Inches(6.92), Inches(11.5), Inches(0.22),
         [[("출처: " + src_text, 8.5, LGRAY, False)]])

def _style_chart(chart, title, y_min=0, y_max=100, show_legend=False, horizontal=False, title_size=12):
    ...
    if show_legend:
        ...
        chart.legend.font.size = Pt(9); chart.legend.font.name = FONT
    ...
    val_axis.tick_labels.font.size = Pt(10); val_axis.tick_labels.font.name = FONT
    cat_axis.tick_labels.font.size = Pt(10); cat_axis.tick_labels.font.name = FONT
...
def bar_chart(slide, l, t, w, h, key, title, y_min=0, y_max=100, include_grok=True, sub=""):
    ...
    _style_chart(chart, title + sub, y_min=y_min, y_max=y_max, horizontal=True, title_size=11.5)
    plot = chart.plots[0]
...
def grouped_bar(slide, l, t, w, h, categories, series_list, title, y_min=0, y_max=100):
    data = CategoryChartData()
    ...
    _style_chart(chart, title, y_min=y_min, y_max=y_max, show_legend=True, title_size=12)
...
text(s, Inches(0.97), Inches(4.15), Inches(11.6), Inches(1.0),
     ...
     line_spacing=1.25)
source_footer(s, "Anthropic · OpenAI · Google · xAI 공식 Model Card + Vellum · Artificial Analysis 독립검증")
pnum(s, 1)
...
for i, (t_, b, c) in enumerate(cards):
    simple_box(s, Inches(0.5 + i * 6.35), Inches(1.58), Inches(6.0), Inches(0.95), t_, b, accent=c)
...
grouped_bar(s, Inches(0.5), Inches(3.42), Inches(12.3), Inches(2.55),
            ...
            "영역별 평균 점수 (%)", y_min=40, y_max=100)

takeaway(s, Inches(0.5), Inches(6.18), Inches(12.35), Inches(0.65),
         "쉽게 말하면",
         ["코딩 = 코드·명령어 잘 다루는지  |  추론 = 머리 쓰는 문제·수학·과학 잘 푸는지"])
source_footer(s, "각 사 공식 System Card 평균치 산출")
pnum(s, 2)
...
# SWE Verified: 70~90 범위 → y_min=60, y_max=100
bar_chart(s, Inches(0.45), Inches(1.82), Inches(5.95), Inches(3.55),
          "swe_verified", "SWE-bench Verified (보통)", y_min=60, y_max=100,
          sub="\n버그 고치기 성공률")

# SWE Pro: 50~70 범위 → y_min=40, y_max=80
bar_chart(s, Inches(6.85), Inches(1.82), Inches(5.95), Inches(3.55),
          "swe_pro", "SWE-bench Pro (어려움)", y_min=40, y_max=80, include_grok=False,
          sub="\nGrok 미공개 — 3종만 비교")

takeaway(s, Inches(0.5), Inches(5.55), Inches(12.35), Inches(1.0),
         "코딩 ① 결론",
         ["보통·어려운 버그 고치기 모두 Claude 1위 (88.6% → 69.2%)",
          "GPT·Gemini도 50% 넘음 = 쓸 만한 수준"])
source_footer(s, "Anthropic System Card (2026.5) · OpenAI · Google 공식 발표")
pnum(s, 3)
...
# Terminal: 68~80 → y_min=60, y_max=85
bar_chart(s, Inches(0.45), Inches(1.82), Inches(5.95), Inches(3.55),
          "terminal", "Terminal-Bench 2.1 (터미널)", y_min=60, y_max=85, include_grok=False,
          sub="\n명령 작업 성공률")

# OSWorld: 73~85 → y_min=65, y_max=90
bar_chart(s, Inches(6.85), Inches(1.82), Inches(5.95), Inches(3.55),
          "osworld", "OSWorld (PC 직접 조작)", y_min=65, y_max=90, include_grok=False,
          sub="\nGrok 미공개")

takeaway(s, Inches(0.5), Inches(5.55), Inches(12.35), Inches(1.0),
         "코딩 ② 결론",
         ["명령어·스크립트 작업 → GPT 1위 (78.2%)",
          "PC 화면 직접 조작 → Claude 1위 (83.4%)"])
source_footer(s, "Anthropic System Card · HundredTabs 벤치마크 비교 (2026.5)")
pnum(s, 4)
...
# GPQA: 86~95 → y_min=80, y_max=100
bar_chart(s, Inches(0.45), Inches(1.82), Inches(5.95), Inches(3.55),
          "gpqa", "GPQA Diamond (과학 퀴즈)", y_min=80, y_max=100,
          sub="\n정답률")

# HLE: 40~52 → y_min=30, y_max=60
bar_chart(s, Inches(6.85), Inches(1.82), Inches(5.95), Inches(3.55),
          "hle_plain", "HLE (도구 없이 혼자 풀기)", y_min=30, y_max=60,
          sub="\n* Grok 근사치")

takeaway(s, Inches(0.5), Inches(5.55), Inches(12.35), Inches(1.0),
         "추론 ① 결론",
         ["과학 퀴즈(GPQA)는 4종 모두 88~94% — 격차 작음",
          "진짜 어려운 생각(HLE)에서는 Claude·Grok 상위"])
source_footer(s, "Anthropic System Card · Vellum 벤치마크 해설 (2026.5)")
pnum(s, 5)
...
text(s, Inches(0.5), Inches(1.12), Inches(12.3), Inches(0.55),
     [[("MATH-500 = 중·고등~대학 초급 수학  |  AIME = 올림피아드급  |  FrontierMath = 최고난도", 11.5, GRAY, False)],
      [("쉬운 수학은 모두 94%↑  |  어려울수록 점수 차이가 커짐", 11.5, GRAY, False)]])

# MATH/AIME: 93~98 vs FrontierMath: 48~55 → 두 축이 다름
# → 상단: 쉬운수학 (y_min=90, y_max=100), 하단: 어려운수학 (y_min=40, y_max=60) 분리

grouped_bar(s, Inches(0.45), Inches(1.78), Inches(5.95), Inches(2.15),
            MODEL_SHORT,
            [
                ("MATH-500", DATA["math500"]),
                ("AIME 2026", DATA["aime"]),
            ],
            "기본·올림피아드 수학 (%)", y_min=90, y_max=100)

grouped_bar(s, Inches(6.85), Inches(1.78), Inches(5.95), Inches(2.15),
            MODEL_SHORT,
            [("FrontierMath", DATA["frontier_math"])],
            "최고난도 수학 (%)", y_min=40, y_max=60)

takeaway(s, Inches(0.5), Inches(4.25), Inches(12.35), Inches(1.0),
         "추론 ② 결론",
         ["일반·올림피아드 수학 → GPT 97% 1위 (Claude·Gemini·Grok도 95% 이상)",
          "최고난도 수학(FrontierMath) → Grok 54% 1위"])
source_footer(s, "AI Magic · Vellum 벤치마크 비교 (2026.5) · FrontierMath 공식")
pnum(s, 6)

# ── 7. HLE 도구 사용 + 종합 ──
s = add_slide(); set_bg(s, WHITE)
header(s, KICKER, "추론 ③ — 도구 사용 시 · 종합 정리")

# HLE with tools: 50~60 → y_min=45, y_max=65
bar_chart(s, Inches(0.45), Inches(1.55), Inches(6.0), Inches(3.2),
          "hle_tools", "HLE — 검색·계산 도구 사용", y_min=45, y_max=65, include_grok=False,
          sub="\n도구 쓰면 점수 상승")

# 종합 winner cards
winners = [
    ("코딩 버그 수정", "Claude", "SWE Pro 69%", C_CLAUDE),
    ("터미널 명령어", "GPT", "Terminal 78%", C_GPT),
    ("PC 직접 조작", "Claude", "OSWorld 83%", C_CLAUDE),
    ("어려운 생각", "Claude", "HLE 50~58%", C_CLAUDE),
    ("과학 퀴즈", "Gemini", "GPQA 94%", C_GEMINI),
    ("최고난도 수학", "Grok", "Frontier 54%", C_GROK),
]
for i, (area, who, score, col) in enumerate(winners):
    ...
    text(s, cx + Inches(0.12), cy + Inches(0.15), Inches(1.65), Inches(1.55),
         [[(area, 10, NAVY, True)], [(who, 14, col, True)], [(score, 9, GRAY, False)]],
         align=PP_ALIGN.CENTER, line_spacing=1.15)

takeaway(s, Inches(0.5), Inches(5.7), Inches(12.35), Inches(0.85),
         "한 줄 요약",
         ["코딩·복잡한 생각 → Claude  |  터미널·수학 → GPT  |  과학 → Gemini  |  초난도 수학 → Grok"])
source_footer(s, "Anthropic System Card · Vellum · AI Magic (2026.5)")
pnum(s, 7)

# ── 8. 출처 ──
s = add_slide(); set_bg(s, WHITE)
header(s, KICKER, "점수는 어디서 가져왔나요?")
text(s, Inches(0.65), Inches(1.45), Inches(12), Inches(4.8),
     [[("1차 — 공식 자료 (각 AI 회사가 직접 공개)", 14, NAVY, True)],
      [("• Anthropic Claude Opus 4.8 System Card (2026.5.28)", 11.5, GRAY, False)],
      [("• OpenAI GPT-5.5 System Card  ·  Google Gemini 3.1 Pro Model Card  ·  xAI Grok 4.3 Model Card", 11.5, GRAY, False)],
      [("", 8, GRAY, False)],
      [("2차 — 독립 검증 (회사 말만 믿지 않고 다른 기관이 확인)", 14, NAVY, True)],
      [("• Artificial Analysis Intelligence Index  ·  Vellum 벤치마크 해설  ·  HundredTabs  ·  AI Magic", 11.5, GRAY, False)],
      [("", 8, GRAY, False)],
      [("꼭 기억할 점", 14, ACCENT, True)],
      [("① 같은 시험이어도 측정 방법이 다르면 점수가 달라질 수 있음", 11.5, GRAY, False)],
      [("② * 표시는 공식 미공개라 근사치 — 보고 시 '약' 또는 '추정'으로 말하기", 11.5, GRAY, False)],
